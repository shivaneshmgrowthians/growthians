...
export default function CEOApprovals() {
  ...
  const loadData = async () => {
    setLoading(true)
    const [leavesRes, logoffsRes] = await Promise.all([
      supabase
        .from('leave_requests')
  ...
  const submitReview = async () => {
    ...
    // Deduct leave balance if approved (only full-day leaves, not half)
    if (
      type === 'leave' &&
      action === 'approve' &&
      req.leave_type === 'full' &&
      req.days_requested > 0
    ) {
  ...
  return (
    <div>
      <PageHeader
        ...
        title="Approvals"
        subtitle="Review and approve team leave and logoff requests"
      />
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            {filteredLeaves.length === 0 ? (
              <EmptyState message="No leave requests in this filter" />
            ) : (
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-black/50 mb-2">
                            {req.users?.designation} · Submitted{' '}
                        ...
                        {req.status === 'pending' && (
                          <div className="flex flex-col gap-2 flex-shrink-0">
                            <Button
                              variant="success"
                              onClick={() => openReview(req, 'leave', 'approve')}
                            ...
                            <Button
                              variant="danger"
                              onClick={() => openReview(req, 'leave', 'reject')}
                              className="!px-3 !py-2 text-xs"
                            >
          ...
          <div>
            ...
            {filteredLogoffs.length === 0 ? (
              <EmptyState message="No logoff requests in this filter" />
            ) : (
              <div className="space-y-3">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        {req.status === 'pending' && (
                          <div className="flex flex-col gap-2 flex-shrink-0">
                            <Button
                              variant="success"
                              onClick={() => openReview(req, 'logoff', 'approve')}
                              className="!px-3 !py-2 text-xs"
                            >
                              <CheckCircle2 className="w-3.5 h-3.5" strokeWidth={2} />
                              Approve
                            ...
                            <Button
                              variant="danger"
                              onClick={() => openReview(req, 'logoff', 'reject')}
                              className="!px-3 !py-2 text-xs"
                            >
                              <XCircle className="w-3.5 h-3.5" strokeWidth={2} />
                              Reject
      ...
      {reviewModal && (
        <Modal
          title={`${reviewModal.action === 'approve' ? 'Approve' : 'Reject'} ${
            reviewModal.type === 'leave' ? 'Leave' : 'Logoff'
          }`}
