...
export default function EmployeeTodaySheet() {
  ...
  useEffect(() => {
    if (profile?.id) { loadData(); loadTeamToday() }
  }, [profile?.id])
  ...
  const loadData = async () => {
    ...
    if (taskData) { setDailyTaskId(taskData.id); setStatus(taskData.status); setFrozenLoginTime(taskData.login_time || null); setFrozenLogoffTime(taskData.logoff_time || null) }
    ...
    if (!taskData && loginTime) {
      const { data: newTask } = await supabase.from('daily_tasks').insert({ user_id: profile.id, date: today, status: 'draft', login_time: loginTime }).select().single()
      if (newTask) { setDailyTaskId(newTask.id); setFrozenLoginTime(loginTime) }
    }
  ...
  const loadTeamToday = async () => {
    ...
    const { data: tasks } = await supabase.from('daily_tasks').select('user_id, status, login_time, logoff_time').eq('date', today).in('user_id', users.map(u => u.id))
    setTeamToday(users.map(u => { const task = tasks?.find(t => t.user_id === u.id); return { ...u, task_status: task?.status || 'not started', login_time: task?.login_time, logoff_time: task?.logoff_time } }))
  }
  ...
  const autosave = useCallback(async (newSlots) => {
    ...
    if (!taskId) {
      const { data, error } = await supabase.from('daily_tasks').insert({ user_id: profile.id, date: today, status: 'draft', login_time: loginTime }).select().single()
      if (error) return; taskId = data.id; setDailyTaskId(taskId); setFrozenLoginTime(loginTime)
    }
  ...
  const handleSubmit = async () => {
    ...
    const totalHours = calculateHours(todaySlots); const logoffTime = getCurrentTime()
    const { error } = await supabase.from('daily_tasks').update({ status: 'submitted', submitted_at: new Date().toISOString(), logoff_time: logoffTime, total_hours: totalHours }).eq('id', dailyTaskId)
    ...
    setStatus('submitted'); setFrozenLogoffTime(logoffTime); showToast(`Day submitted · ${totalHours} hours logged`); setSubmitting(false); loadWeeklyHours(); loadAnalytics()
  }

  const handleRevert = async () => {
    setReverting(true)
    const { error } = await supabase.from('daily_tasks').update({ status: 'draft', submitted_at: null, logoff_time: null, total_hours: null }).eq('id', dailyTaskId)
    if (error) { showToast('Failed to revert', 'error'); setReverting(false); return }
    setStatus('draft'); setFrozenLogoffTime(null); showToast('Reverted to draft'); setReverting(false)
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <StatBlock icon={Clock} label="Login" value={displayLoginTime} />
        <StatBlock icon={Timer} label="Today" value={`${todayHours}h`} accent />
        <StatBlock icon={TrendingUp} label="Weekly" value={`${weeklyHours}h`} subtitle="resets Mon" />
        <StatBlock icon={ListTodo} label="Slots" value={`${filledCount}/${todaySlots.length}`} />
      </div>
      ...
      {submitted && (
        <div className="mb-6 bg-[#C5F542] p-3 sm:p-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-black text-sm">Submitted · {todayHours}h logged</div>
            <div className="text-xs text-black/70 truncate">In: {displayLoginTime} · Out: {frozenLogoffTime || '—'}</div>
          </div>
        </div>
      )}
        ...
        <div className="border-2 border-t-0 border-black/20 divide-y divide-black/10">
          {todaySlots.map((slot) => (
            <div key={slot.slot_index} className="group">
              ...
              <textarea
                disabled={submitted}
                value={slot[mobileTabField]}
                onChange={(e) => updateSlot(slot.slot_index, mobileTabField, e.target.value)}
                className="w-full px-3 py-3 text-sm bg-white focus:outline-none disabled:text-black resize-none"
                rows={3}
                placeholder={mobileTabPlaceholder}
              />
            </div>
          ))}
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {teamToday.map((u) => {
              ...
              return (
                <button key={u.id} onClick={() => openSpectate(u)} className="bg-white border border-black/10 p-4 text-left hover:border-black/30">
                  ...
                  <span className={`text-[10px] uppercase tracking-widest font-semibold px-2 py-0.5 ${u.task_status === 'submitted' ? 'bg-[#C5F542] text-black' : u.task_status === 'draft' ? 'bg-yellow-100 text-yellow-700' : 'bg-black/5 text-black/40'}`}>
            ...
            <div>
              {spectateTask && (
                <div className="flex items-center gap-3 mb-3 text-xs text-black/50 flex-wrap">
                  {spectateTask.login_time && <span>In: <strong className="text-black">{spectateTask.login_time}</strong></span>}
                  {spectateTask.logoff_time && <span>Out: <strong className="text-black">{spectateTask.logoff_time}</strong></span>}
...
function StatBlock({ icon: Icon, label, value, subtitle, accent }) {
  return (
    <div className={`p-3 sm:p-4 border ${accent ? 'bg-black text-white border-black' : 'bg-white border-black/10'}`}>
      <div className="flex items-center gap-1.5 mb-1.5">
        <Icon className={`w-3.5 h-3.5 ${accent ? '' : 'text-black/40'}`} style={accent ? { color: '#C5F542' } : {}} strokeWidth={1.8} />
        <span className={`text-[9px] sm:text-[10px] uppercase tracking-widest font-semibold ${accent ? 'text-white/60' : 'text-black/50'}`}>{label}</span>
      </div>
      <div className="text-xl sm:text-2xl font-bold">{value}</div>
      {subtitle && <div className={`text-[9px] sm:text-[10px] mt-1 ${accent ? 'text-white/50' : 'text-black/50'}`}>{subtitle}</div>}
    </div>
  )
}